...
static int PrintFrameInfo(struct mad_header *Header)
{
  ...
  ((MAD_VERSION_MAJOR==0) && (MAD_VERSION_MINOR>=15))
    ...
    printf("%lu kb/s audio MPEG layer %s stream at %dHz Emphasis: %s Mode: %s\n", Header->bitrate / 1000, Layer, Header->samplerate, Emphasis, Mode);
  ...
  if (Frame->header.mode != MAD_MODE_SINGLE_CHANNEL) {
    for (Sample = 0; Sample < Samples; Sample++) {
      for (SubBand = 0; SubBand < 32; SubBand++) {
        ...
        Frame->sbsample[1][Sample][SubBand] = mad_f_mul(Frame->sbsample[1][Sample][SubBand], Filter[SubBand]);
      }
    }
  } else {
    for (Sample = 0; Sample < Samples; Sample++) {
...
int MP3_Stop()
{
    ...
    //clear buffer
    memset(OutputBuffer, 0, sizeof(short)*OUTPUT_BUFFER_SIZE);
